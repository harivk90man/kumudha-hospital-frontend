...
-- have schema-level USAGE on `public`. The Spring backend connects with
-- the `postgres` superuser (or a dedicated service role) — no PostgREST
-- exposure. Revoke everything from anon / authenticated so a
-- misconfigured client cannot read the tables.
--
-- Safe to run on plain Postgres without these roles — DO blocks check
-- pg_roles first.
-- =====================================================================
...
begin
  ...
  -- Revoke from service_role too — Supabase's elevated PostgREST role.
  if exists (select 1 from pg_roles where rolname = 'service_role') then
    execute 'revoke all on schema public from service_role';
    ...
    execute 'alter default privileges in schema public revoke all on sequences from service_role';
    execute 'alter default privileges in schema public revoke all on functions from service_role';
  end if;
